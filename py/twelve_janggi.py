"""Twelve Janggi (십이장기) — a GameEngine subclass (see engine_api.py).
...
class TwelveJanggi(GameEngine):
    ...
    def goal_row(self, owner):
        """The opponent-territory row that `owner` wants to reach."""
        return 0 if owner == 0 else self.ROWS - 1

    def dirs_for(self, piece_type, owner):
        base = self.MOVES[piece_type]
        return base if owner == 0 else tuple((-r, -c) for r, c in base)

    def initial_state(self, first=0):
        board = [None] * (self.ROWS * self.COLS)

        def put(r, c, t, o):
            board[r * self.COLS + c] = (t, o)
    ...
    def piece_moves_at(self, st, idx):
        p = st['board'][idx]
        if p is None:
            return []
        t, owner = p
        ...
        out = []
        for dr, dc in self.dirs_for(t, owner):
            nr, nc = r + dr, c + dc
            if not (0 <= nr < self.ROWS and 0 <= nc < self.COLS):
                continue
            tgt = st['board'][nr * self.COLS + nc]
            if tgt is not None and tgt[1] == owner:
                continue
            out.append(nr * self.COLS + nc)
        return out

    def drop_idxs(self, st, player):
        forbidden = self.goal_row(player)
        return [i for i, p in enumerate(st['board'])
                if p is None and i // self.COLS != forbidden]
    ...
    def legal_moves(self, st):
        ...
        for i, p in enumerate(st['board']):
            if p is not None and p[1] == player:
                for to in self.piece_moves_at(st, i):
                    out.append({'from': i, 'to': to})
        if st['hands'][player]:
            empties = self.drop_idxs(st, player)
            for t in sorted(set(st['hands'][player])):
                for to in empties:
                    out.append({'drop': t, 'to': to})
        return out

    def has_move_for(self, st, player):
        if st['hands'][player] and self.drop_idxs(st, player):
            return True
        return any(p is not None and p[1] == player and self.piece_moves_at(st, i)
                   for i, p in enumerate(st['board']))
    ...
    def apply_move(self, st, mv):
        ...
        if 'drop' in mv:
            ...
            if p[0] == 'ja' and mv['to'] // self.COLS == self.goal_row(player):
        ...
        st['pending_try'] = None
        for i, p in enumerate(st['board']):
            if p == ('wang', player) and i // self.COLS == self.goal_row(player):
                st['pending_try'] = player
                break
        st['turn'] = 1 - player
        if not self.has_move_for(st, st['turn']):
    ...
    def evaluate(self, st, me):
        ...
        for i, p in enumerate(st['board']):
            ...
            v = self.VAL[t]
            if t == 'ja':
                v += 2 * ((3 - i // self.COLS) if owner == 0 else (i // self.COLS))
            s += v if owner == me else -v
        for owner in (0, 1):
            hv = sum(self.HAND_VAL[t] for t in st['hands'][owner])
    ...
    def order_moves(self, st, moves):
        ...
        def key(mv):
            if 'drop' not in mv:
                cap = st['board'][mv['to']]
                if cap is not None:
                    return -(10_000 if cap[0] == 'wang' else self.VAL[cap[0]])
            return 0
