'use strict';
/**
 * Twelve Janggi (십이장기) — a GameEngine subclass (see core/engine.js).
 *
 ...
 *     no legal move loses.
 *
 * State: { board:[{type,owner}|null]*12, hands:[[],[]], turn, pendingTry,
 *          winner, reason }. Moves: {from, to} | {drop: pieceType, to}.
 * Board squares are indexed row-major 0..11 with row 0 at the top.
 *
 * Mirrors py/twelve_janggi.py line for line; both must stay rule-identical.
 */
const TwelveJanggi = (() => {
  const ROWS = 4, COLS = 3;
  ...
  class TwelveJanggi extends GameEngine {
    constructor() {
      super();
      // constants + display metadata the UI reads off the engine
      Object.assign(this, { ROWS, COLS, MOVES, HANJA, KOR, rules });
    }

    goalRow(owner) { return owner === 0 ? 0 : ROWS - 1; }
    ...
    pieceMovesAt(st, idx) {
      ...
      for (const [dr, dc] of this.dirsFor(p.type, p.owner)) {
    ...
    dropIdxs(st, player) {
      ...
      for (let i = 0; i < ROWS * COLS; i++) {
        if (!st.board[i] && Math.floor(i / COLS) !== this.goalRow(player)) out.push(i);
      }
      return out;
    }
    ...
    legalMoves(st) {
      ...
      for (let i = 0; i < ROWS * COLS; i++) {
        const p = st.board[i];
        if (p && p.owner === player) {
          for (const to of this.pieceMovesAt(st, i)) out.push({ from: i, to });
        }
      }
      if (st.hands[player].length) {
        const empties = this.dropIdxs(st, player);
    ...
    hasMoveFor(st, player) {
      if (st.hands[player].length && this.dropIdxs(st, player).length) return true;
      return st.board.some((p, i) => p && p.owner === player && this.pieceMovesAt(st, i).length);
    }
    ...
    applyMove(st, mv) {
      ...
      if (mv.drop) {
        ...
        if (p.type === 'ja' && Math.floor(mv.to / COLS) === this.goalRow(player)) {
      ...
      for (let i = 0; i < ROWS * COLS; i++) {
        const p = st.board[i];
        if (p && p.type === 'wang' && p.owner === player && Math.floor(i / COLS) === this.goalRow(player)) {
          st.pendingTry = player;
          break;
        }
      }
      st.turn = 1 - player;
      if (!this.hasMoveFor(st, st.turn)) { st.winner = player; st.reason = 'stuck'; }
    }
